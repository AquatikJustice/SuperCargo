...
/** Can a footprint (fwxfl on the x/z plane, fh tall) sit at local (x,y,z)? */
function canPlace(
  s: GridState,
  x: number,
  y: number,
  ...
  // support: on the floor, or every cell directly below is filled
  if (y > 0) {
    for (let dz = 0; dz < fl; dz++)
      for (let dx = 0; dx < fw; dx++) if (!s.occ[idx(g, x + dx, y - 1, z + dz)]) return false
  }
  return true
}

function fill(s: GridState, x: number, y: number, z: number, fw: number, fl: number, fh: number): void {
  const g = s.grid
  for (let dy = 0; dy < fh; dy++)
    for (let dz = 0; dz < fl; dz++)
      for (let dx = 0; dx < fw; dx++) s.occ[idx(g, x + dx, y + dy, z + dz)] = 1
  s.used += fw * fl * fh
}
    ...
    for (let y = 0; y + h <= g.h; y++) {
      for (let x = 0; x < g.w; x++) {
        for (const [fw, fl, rotated] of orients) {
          if (canPlace(s, x, y, z, fw, fl, h)) return { x, y, z, fw, fl, rotated }
        }
      }
    }
  }
...
/**
 * Pack `boxes` into a ship's `grids`, ONE DESTINATION AT A TIME so each stop gets
 * its own contiguous SECTION with a 1-cell gap before the next - easier to read
 * and load than colour-coding a single intermixed pile. Stops are laid in
 * delivery order from the front (first drop-off nearest the ramp); a section that
 * fills one bay spills into the next. Reference-only grids (autoLoad === false)
 * are skipped and grids reject boxes larger than their maxSize.
 */
export function packCargo(grids: CargoGrid[], boxes: PackBox[]): PackResult {
  ...
  // Group boxes by stop, stops ascending (first delivery first), largest first
  // within a stop so big boxes anchor the bottom of their section.
  ...
  for (const stopIdx of stops) {
    ...
    for (const box of stopBoxes) {
      ...
      for (let gi = startGi; gi < states.length; gi++) {
        ...
        const minZ = gi === startGi ? startZ : 0
        const spot = findSpot(s, dims.w, dims.l, dims.h, minZ)
        if (!spot) continue
        fill(s, spot.x, spot.y, spot.z, spot.fw, spot.fl, dims.h)
        placements.push({
          box,
          gridId: s.grid.id,
          x: spot.x,
    ...
    // Advance the frontier past this section, leaving a 1-cell gap. If the gap
    // would run off the end of the bay, start the next section in the next bay.
    if (endZ + 1 < states[endGi]?.grid.l) {
      frontGi = endGi
      frontZ = endZ + 1
    } else {
      frontGi = endGi + 1
